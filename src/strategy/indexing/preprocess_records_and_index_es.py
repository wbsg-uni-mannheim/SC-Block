...
from src.strategy.retrieval.retrieval_strategy import load_es_index_configuration
...
@click.option('--switched', default=False, help='Switched table A and B')
def load_data(dataset, worker, tokenizer, no_test, with_language_detection, duplicate_check, entity_length_check, switched):
    ...
    if 'ES_INSTANCE' in os.environ:
        ...
        # default to process records in file system
        logger = logging.getLogger()
    ...
    entity_index_name = determine_es_index_name(dataset, tokenizer=tokenizer, switched=switched)
    processed_records = [] # Store processed records to reduce dependency on elastic search

    #time.sleep(5)
    if no_test and _es is not None:
    ...
    if switched:
        # Table A and B are switched!
        directory = '{}/corpus/switched/{}'.format(os.environ['DATA_DIR'], dataset)
    else:
    ...
    while True:
        ...
        results, entity_index, processed_records = send_actions_to_elastic(_es, results, processed_records, entity_index, index_statistics, pbar, no_test, worker)
    ...
    # Create dataframe from processed actions and store it
    df_processed_records = pd.DataFrame(processed_records)
    if switched:
        directory = '{}/processed_records/switched/{}'.format(os.environ['DATA_DIR'], dataset)
    else:
        directory = '{}/processed_records/{}'.format(os.environ['DATA_DIR'], dataset)
    create_directory_if_not_exists(directory)
    logger.info(df_processed_records.head())
    df_processed_records.to_pickle('{}/{}_processed_records.pkl'.format(directory, dataset))
...
def send_actions_to_elastic(_es, results, processed_actions, entity_index, index_statistics, pbar, no_test, worker):
    ...
    if len(actions) > 0 and no_test:
        ...
        processed_actions.extend([prepare_processed_action(action) for action in actions])

    # Remove collected results from list of results
    results = [result for result in results if result not in collected_results]

    return results, entity_index, processed_actions


def prepare_processed_action(action):
    processed_action = action['_source']
    processed_action['id'] = action['_id']
