...
@click.option('--switched', default=True, help='Switched table A and B')
def load_data(dataset, worker, switched):
    ...
    doc_index_number = 1
    if switched:
        table_index_name = '{}_{}_switched'.format('table_index', dataset)
    else:
        table_index_name = '{}_{}'.format('table_index', dataset)
    ...
    if switched:
        ...
        directory = '{}/corpus/switched/{}'.format(path_to_data_dir, dataset)
    else:
        directory = '{}/corpus/{}'.format(path_to_data_dir, dataset)
    pool = Pool(worker)
