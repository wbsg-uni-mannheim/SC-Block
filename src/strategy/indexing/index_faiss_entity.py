...
from src.strategy.indexing.faiss_collector import FaissIndexCollector
...
@click.option('--batch_size', default=4096)
@click.option('--serialization', default='attribute_names')
@click.option('--switched', default=False, help='Switched table A and B --> If False Table B is indexed')
def load_data(path_to_config, dataset, bi_encoder_name, model_name, base_model, with_projection, pooling, similarity_measure, dimensions, clusters, batch_size, serialization, switched):
    ...
    if path_to_config is not None:
        ...
        without_special_tokens_and_attribute_names = True if serialization == 'no_attribute_names' else False
        logger.info('Without_special_tokens_and_attribute_names: {}'.format(without_special_tokens_and_attribute_names))
        bi_encoder_configuration = {'name': bi_encoder_name, 'model_name': model_name, 'base_model': base_model,
                                    ...
                                    'dimensions': dimensions, 'without_special_tokens_and_attribute_names': without_special_tokens_and_attribute_names }

    logger.info('Chosen Model {} for indexing dataset {}'.format(model_name, dataset))

    start_time = time.time()

    # Load bi-encoder
    strategy = QueryByEntity(dataset, switched=switched)
    entity_index_name = determine_es_index_name(dataset, clusters=clusters, switched=switched)

    if 'ES_INSTANCE' in os.environ:
        # Rely on ES index to determine number of entities
        ...
        # Rely on pandas dataframe to determine number of entities
        logger.info('Create FAISS index for pandas dataframe of dataset {}'.format(dataset))
        no_entities = len(strategy.processed_records)
        final_step = int(no_entities / batch_size) + 1
    ...
    faiss_collector = FaissIndexCollector(dataset, model_name, pooling, similarity_measure, final_step,
                                          dimensions, clusters, switched=switched)
    ...
    for i in tqdm(range(final_step)):
        ...
        #print(entities[0])
        if len(entities) == 0:
            logger.warning('No entities retrieved from {}!'.format(entity_index_name))
            logger.warning('Start: {} - End: {}'.format(start, end))
            # Add dummy entities
            entities = [{'id': f'dummy{start+j}', 'text': 'dummy'} for j in range(0, end - start)]
...
def generate_embeddings(bi_encoder_configuration, dataset, input_q, output_q, gpu_n):
    ...
    while True:
        ...
        pooled_outputs = bi_encoder.encode_entities_and_return_pooled_outputs(entities,
                                                                              without_special_tokens_and_attribute_names=bi_encoder_configuration['without_special_tokens_and_attribute_names'])
