...
class QueryByEntity(RetrievalStrategy):
    ...
    def __init__(self, schema_org_class, clusters=False, rank_evidences_by_table=False, all_attributes=False, tokenizer=None):
        ...
        if self.all_attributes:
            self.entity_serizalizer = EntitySerializer(schema_org_class)
    ...
    def retrieve_evidence(self, query_table, evidence_count, entity_id):
        ...
        for row in query_table.table:
            ...
            if entity_id is not None and entity_id != row['entityId'] and not self.rank_evidences_by_table:
                continue
            index_name = determine_es_index_name(self.schema_org_class, clusters=self.clusters, tokenizer=self.tokenizer)
            if self.all_attributes:
