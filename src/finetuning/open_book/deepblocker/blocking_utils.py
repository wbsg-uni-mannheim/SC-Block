...
def compute_blocking_statistics(candidate_set_df, golden_pos_df, golden_all_df, left_df, right_df):
    ...
    merged_df = pd.merge(candidate_set_df, golden_pos_df, on=['ltable_id', 'rtable_id'])
    merged_all_df = pd.merge(candidate_set_df, golden_all_df, on=['ltable_id', 'rtable_id'])

    candidate_set_df.sort_values(by=['ltable_id', 'rtable_id'], ascending=[False, False], inplace=True)
    print(candidate_set_df.head(100))

    golden_pos_df.sort_values(by=['ltable_id', 'rtable_id'], ascending=[False, False], inplace=True)
    print(golden_pos_df.head(100))
    ...
    statistics_dict = {
        ...
        "recall": len(merged_df) / len(golden_pos_df) if len(golden_pos_df) > 0 else 0,
        "precision": len(merged_df) / len(merged_all_df) if len(merged_all_df) > 0 else 0, # Precision based on all (positive/negative) pairs
